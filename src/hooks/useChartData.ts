...
export const defaultIndicators: ChartIndicator[] = [
  { id: "sma20", name: "Simple Moving Average (20)", shortName: "SMA 20", type: "overlay", enabled: true, color: "hsl(217, 91%, 60%)", params: { period: 20 } },
  { id: "sma50", name: "Simple Moving Average (50)", shortName: "SMA 50", type: "overlay", enabled: false, color: "hsl(38, 92%, 50%)", params: { period: 50 } },
  { id: "ema12", name: "Exponential Moving Average (12)", shortName: "EMA 12", type: "overlay", enabled: false, color: "hsl(262, 83%, 58%)", params: { period: 12 } },
  { id: "ema26", name: "Exponential Moving Average (26)", shortName: "EMA 26", type: "overlay", enabled: false, color: "hsl(160, 84%, 45%)", params: { period: 26 } },
  { id: "bb", name: "Bollinger Bands", shortName: "BB", type: "overlay", enabled: false, color: "hsl(180, 100%, 50%)", params: { period: 20, stdDev: 2 } },
  { id: "vwap", name: "Volume Weighted Avg Price", shortName: "VWAP", type: "overlay", enabled: false, color: "hsl(45, 93%, 47%)", params: {} },
  { id: "rsi", name: "Relative Strength Index", shortName: "RSI", type: "oscillator", enabled: false, color: "hsl(262, 83%, 58%)", params: { period: 14 } },
  { id: "macd", name: "MACD", shortName: "MACD", type: "oscillator", enabled: false, color: "hsl(217, 91%, 60%)", params: { fast: 12, slow: 26, signal: 9 } },
];
...
export function useChartData(symbol: string, timeframe: TimeframeType) {
  ...
  const fetchCandles = useCallback(async () => {
    ...
    try {
      ...
      console.log(`[ChartData] Fetching ${symbol} with timeframe ${timeframe}: resolution=${config.resolution}, days=${config.days}`);
      ...
      console.log(`[ChartData] Received ${data.candles?.length || 0} candles for ${symbol}`);
  ...
  return { candles, loading, error, refetch: fetchCandles, timeframeConfig: timeframeConfig[timeframe] };
}
